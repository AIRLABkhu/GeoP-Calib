...
def create_voxel_grid(all_points, s):
    ...
    max_coords = np.max(all_points, axis=0) + 1e-8  
    ...
    points = origin + (occupied_indices + 0.5) * s 
    return points
        ...
        with open(perspective_path, 'r') as f:
            ...
            for line in f:
                ...
                if param_type == "S":
                    ...
                    calib[cam_key]["K"] = np.array(value).reshape(3, 3)  
                    calib[cam_key]["K"][2,2] = 1.
                elif param_type == "D":
                    calib[cam_key]["D"] = np.array(value) 
                elif param_type == "R":
                    calib[cam_key]["R"] = np.array(value).reshape(3, 3) 
                elif param_type == "T":
                    calib[cam_key]["T"] = np.array(value) 
        ...
        with open(cam_to_pose_path, 'r') as f:
            for line in f:
                if line.startswith("image"):
                    ...
                    calib[f"T_body_{cam_id}"] = matrix_pose 
