...
def fit_gaussians_in_voxels_old(points: torch.Tensor,
    ...
    if single_point_mask.any():
        ...
        voxel_covariances[single_point_mask] = default_cov_matrix  
...
def estimate_density_knn(points, k=16):

    with torch.no_grad():
        dists = torch.cdist(points, points)              # (N,N)
        knn = torch.topk(dists, k=k+1, largest=False).values[:, 1:]  
...
def fit_gaussians_in_voxels(points: torch.Tensor,
    ...
    """
    NOTE:
    - 함수 시그니처 / 반환 형식은 voxel 버전과 완전히 동일
    - 내부 구현만 '중심점 추정 + 그룹화' 방식으로 교체
    """
    ...
    if isinstance(voxel_size, (float, int)):
        ...
        raise TypeError("voxel_size (float or torch.Tensor)")

    if voxel_size.min() <= 0:
        raise ValueError("voxel_size0")
    ...
    num_gaussians = unique_linear_indices.shape[0]

    density = estimate_density_knn(points, k=16)  
    density = density / (density.mean() + 1e-6)    

    centers = torch.empty((num_gaussians, 3), device=device, dtype=dtype)


    centers[0] = points[torch.argmax(density)]

    dist = torch.full((N,), float("inf"), device=device)

    for i in range(1, num_gaussians):

        d = torch.sum((points - centers[i-1]) ** 2, dim=1)
        dist = torch.minimum(dist, d)

        score = dist * density

        centers[i] = points[torch.argmax(score)]


    dists = torch.cdist(points, centers)           # (N, G)
    inverse_indices = torch.argmin(dists, dim=1)   # (N,)
    counts = torch.bincount(inverse_indices, minlength=num_gaussians)


    sum_points_per_group = torch.zeros((num_gaussians, 3), device=device, dtype=dtype)
    sum_points_per_group.index_add_(0, inverse_indices, points)

    gaussian_means = sum_points_per_group / counts.clamp_min(1).unsqueeze(1)


    gaussian_covariances = torch.zeros((num_gaussians, 3, 3), device=device, dtype=dtype)

    single_point_mask = (counts == 1)
    if single_point_mask.any():
        gaussian_covariances[single_point_mask] = (
            torch.eye(3, device=device, dtype=dtype) * default_cov_diag_value
        )

    multi_point_mask = (counts > 1)
    if multi_point_mask.any():
        points_centered = points - gaussian_means[inverse_indices]
        outer_products = points_centered.unsqueeze(2) * points_centered.unsqueeze(1)

        sum_outer = torch.zeros((num_gaussians, 3, 3), device=device, dtype=dtype)
        sum_outer.index_add_(0, inverse_indices, outer_products)

        gaussian_covariances[multi_point_mask] = (
            sum_outer[multi_point_mask] /
            (counts[multi_point_mask] - 1).unsqueeze(-1).unsqueeze(-1)
        )

    return gaussian_means, gaussian_covariances