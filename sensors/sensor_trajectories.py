...
class SensorTrajectories:
    ...
    def get_extrinsic_error_dh(self):
        # gt_rotation_matrix_cl = pytorch3d.transforms.quaternion_to_matrix(self.gt_rotation_cl.cpu()).numpy()
        # curr_rotation_matrix_cl = pytorch3d.transforms.quaternion_to_matrix(self.rotation_cl.detach().cpu()).numpy()
        gt_translation_cl = self.gt_translation_cl.cpu().numpy()
        curr_translation_cl = self.translation_cl.detach().cpu().numpy()
        ...
        eps=1e-7
        pred_q, gt_q = self.rotation_cl.detach().cpu(), self.gt_rotation_cl.detach().cpu()
        pred_q = pred_q / (torch.norm(pred_q, dim=-1, keepdim=True) + eps)
        gt_q = gt_q / (torch.norm(gt_q, dim=-1, keepdim=True) + eps)
        R_pred = pytorch3d.transforms.quaternion_to_matrix(pred_q)
        R_gt = pytorch3d.transforms.quaternion_to_matrix(gt_q)
        R_diff = torch.matmul(R_gt, R_pred.transpose(-1, -2))
        trace = R_diff.diagonal(dim1=-2, dim2=-1).sum(-1)
        val = (trace - 1) / 2
        val = torch.clamp(val, -1.0, 1.0)

        error_rad = torch.acos(val)
        geodesic_error_deg = torch.rad2deg(error_rad)

        return geodesic_error_deg, torch.Tensor(relative_translation_error)
